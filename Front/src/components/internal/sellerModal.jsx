...
export default function SellerModal({ seller, onClose, onSave }) {
	const initial = useMemo(() => {
		...
		if (!seller) return base;
		return { ...base, ...seller, password: "" };
	}, [seller]);
	...
	const handleSubmit = async (e) => {
		...
		if (!seller && (form.password || "").length < 6) {
		...
		setSaving(true);
		try {
			await onSave({ ...form, id: seller?.id });
		} catch (err) {
			...
			<AnimatePresence>
				...
				>
					<div className="sticky top-0 bg-gradient-to-r from-[#1e1608] to-[#2b2010] rounded-t-3xl border-b border-[#d6ab4a]/30 flex items-center justify-between p-6 z-10">
						<h2 className="text-xl font-bold text-[#f5e7c0]">{seller ? "Editar Vendedor" : "Novo Vendedor"}</h2>
						<button
							type="button"
							onClick={onClose}
					...
					<form onSubmit={handleSubmit} className="p-6 space-y-5 bg-[#fffdf8]">
						...
						<div className="space-y-2">
							<p className="text-sm font-medium text-[#4a3918]">Tipo de vendedor</p>
